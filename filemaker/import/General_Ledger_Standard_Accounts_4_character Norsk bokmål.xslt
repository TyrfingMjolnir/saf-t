...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="/*">
    <FMPXMLRESULT xmlns="http://www.filemaker.com/fmpxmlresult">
      ...
      <DATABASE DATEFORMAT="M/d/yyyy" LAYOUT="BudgetCode" NAME="Accounting" RECORDS="{count(/StandardAccounts/Account)}" TIMEFORMAT="h:mm:ss a"/>
      <METADATA>
  ...
  <xsl:template match="*[name() = 'Account']">
    <ROW xmlns="http://www.filemaker.com/fmpxmlresult" MODID="0" RECORDID="{position()}">
      ...
      <COL>
        <DATA>
          <xsl:value-of select="Description[@ISOLanguageCode='NOB']"/>
        </DATA>
      </COL>
    </ROW>
  </xsl:template>
